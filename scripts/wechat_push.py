...
def alert_task_failure(task_name, logical_date, attempt, max_attempts, reason, log_path="", tail=""):
    """发送去重的任务失败告警；告警失败不影响任务本身结果。"""
    try:
        from task_state import TaskState
        state = TaskState(ROOT / "data" / "task_state.sqlite3")
        signature = f"{reason}|{tail[-500:]}"
        if not state.alert_once(task_name, logical_date, signature):
            print(f"[wechat_alert] duplicate skipped: {task_name} {logical_date}")
            return False
        body = (f"超级智能体任务失败\n任务：{task_name}\n日期：{logical_date}\n"
        ...
        if tail:
            body += f"\n日志尾部：\n{tail[-1500:]}"
        return wechat_push(f"超级智能体告警：{task_name}", body)
    except Exception as exc:
        print(f"[wechat_alert] failed: {exc}")
        return False
