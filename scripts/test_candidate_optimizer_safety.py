...
class FakeCore:
    def run_optimization(self, *, output_path, **_):
        output = {"results": [row("bull"), row("bear"), row("sideways")]}
...
before = hashlib.sha256(main_params.read_bytes()).hexdigest()
...
after = hashlib.sha256(main_params.read_bytes()).hexdigest()
assert before == after
