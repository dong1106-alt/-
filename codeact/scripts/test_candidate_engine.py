...
good = {"results": [row("bull"), row("bear"), row("sideways")]}
assert evaluate_backtest(good)["decision"] == "shadow_ready"

few_trades = {"results": [row("bull", 13), row("bear", 13), row("sideways", 13)]}
assert evaluate_backtest(few_trades)["decision"] == "rejected"

bad_drawdown = {"results": [row("bull", dd=-11), row("bear"), row("sideways")]}
assert evaluate_backtest(bad_drawdown)["decision"] == "rejected"

bad_sharpe = {"results": [row("bull", sharpe=1.05), row("bear"), row("sideways")]}
